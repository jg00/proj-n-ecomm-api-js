## E-Commerce API backend only project

E-Commerce API backend only project. Auth, User, Product, Review, Order models.

<img width="400" alt="ecomm-api" src="https://user-images.githubusercontent.com/43181662/168678978-a8af1ff4-b16b-4325-8bf7-fe4edf0cfc81.png">
...
[E-Commerce API Heroku URL](https://proj-n-ecomm-api-js.herokuapp.com)
...
- My notes - Instead of sending back the token with our response and then the frontend storing the token in local storage, now we will attach a cookie to our response and we will store the JWT token in the cookie. Now what is nifty is the fact that the cookie is HttpOnly which means only the browser can access that token which is in the cookie and also with the next request the cookie will be automatically sent as well. This means we dont need to do extra setup on the frontend to manually send the token in the request headers.
